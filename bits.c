...
int allOddBits(int x) {
  return !(((((170<<8)+170)<<16)+((170<<8)+170))&(~x));
}
/* 
 * negate - return -x 
 *   Example: negate(1) = -1.
 ...
 *   Rating: 3
 */
int isLessOrEqual(int x, int y) {//fault
  return !((y+((~x)+1))>>31);
}
//4
/* 
...
int howManyBits(int x) {
  int sign=x^((!(x>>31))+(~0));//we want to figure out the first '1' in the bit vector
  int temp1=(!!(sign>>16))<<4;
  int step1=(!!temp1)+(~0);
  int temp2=(!!(sign>>(24+(step1<<4))))<<3;
  int step2=(!!temp2)+(~0);
  int temp3=(!!(sign>>(28+(step1<<4)+(step2<<3))))<<2;
  int step3=(!!temp3)+(~0);
  int temp4=(!!(sign>>(30+(step1<<4)+(step2<<3)+(step3<<2))))<<1;
  int step4=(!!temp4)+(~0);
  int temp5=(!!(sign>>(31+(step1<<4)+(step2<<3)+(step3<<2)+(step4<<1))));
  int temp_sum=temp1+temp2+temp3+temp4+temp5;
  int last_bit=(sign>>temp_sum)&1;
  return temp_sum+1+last_bit;
}//16+8+4+2+1+2
//float
/* ][]
